...
public class SignIn_Action {
    ...
    public static void Execute(int iTestCaseRow) throws Exception{
    	...
    	LogClass.info("Click action is perfromed on login button" );
        ...
        LogClass.info(sUserName+" is entered in UserName text box" );

        String sPassword = ExcelUtils.getCellData(iTestCaseRow, Constant.Col_Password);

        LogIn_Page.txtbx_Password().sendKeys(sPassword);

        LogClass.info(sPassword+" is entered in Password text box" );

        LogIn_Page.btn_LogIn().click();

        LogClass.info("Click action is performed on Submit button");
